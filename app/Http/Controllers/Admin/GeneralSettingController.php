...
class GeneralSettingController extends Controller
{
    ...
    {
        $edit_data = GeneralSetting::find(2);
    ...
    public function update(Request $request)
    {
        ...
        $updateData = GeneralSetting::find($request->id);
        $input = $request->all();
    
        // Process and save new white logo
        $input['white_logo'] = $this->processAndSaveImage($request->file('white_logo'), 'public/uploads/settings/', $updateData->white_logo,'', '');
    
        // Process and save new dark logo
        $input['dark_logo'] = $this->processAndSaveImage($request->file('dark_logo'), 'public/uploads/settings/', $updateData->dark_logo,'', '');
    
        // Process and save new favicon image
        $input['favicon'] = $this->processAndSaveImage($request->file('favicon'), 'public/uploads/settings/', $updateData->favicon, 32, 32);
    
        // Process and save new payment_method image
        $input['payment_method'] = $this->processAndSaveImage($request->file('payment_method'), 'public/uploads/settings/', $updateData->payment_method, 32, 32);
    
        // Process and save new qr_code image
        $input['qr_code'] = $this->processAndSaveImage($request->file('qr_code'), 'public/uploads/settings/', $updateData->qr_code, 32, 32);
    
        // Process and save new play_store image
        $input['play_store'] = $this->processAndSaveImage($request->file('play_store'), 'public/uploads/settings/', $updateData->play_store, 32, 32);
    
        // Process and save new app_store image
        $input['app_store'] = $this->processAndSaveImage($request->file('app_store'), 'public/uploads/settings/', $updateData->app_store, 32, 32);
    ...
    private function processAndSaveImage($file, $uploadPath, $oldImageUrl = null, $width = null, $height = null)
    {
        if (!$file) {
            return $oldImageUrl;
        }
    
        $name = time() . '-' . pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME) . '.webp';
        $name = strtolower(preg_replace('/\s+/', '-', $name));
        $imageUrl = $uploadPath . $name;
