                ...
                <form action="{{route('blogs.update')}}" method="POST" class=row data-parsley-validate=""  enctype="multipart/form-data">
                    ...
                    <div class="col-sm-12">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control @error('author') is-invalid @enderror" name="author" value="{{$edit_data->author}}" id="author" required="">
                            @error('author')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <label for="category_id" class="form-label">Blog Category</label>
                             <select class="form-control select2-multiple @error('category_id') is-invalid @enderror" name="category_id" data-toggle="select2"  data-placeholder="Choose ...">
                                <optgroup>
                                    ...
                                    <option  value="{{$value->id}}" @if($edit_data->category_id==$value->id)selected @endif>{{$value->title}}</option>
                                    @endforeach
                                </optgroup>
                            </select>
                            @error('category_id')
